...
hook OnPlayerGiveDamageActor(playerid, damaged_actorid, Float:amount, weaponid, bodypart)
{
    if(damaged_actorid == gColonelActorID[playerid])
    {
        ...
        GameTextForPlayer(playerid, "~r~misi gagal!", 9000, 0);
...
hook OnPlayerEnterCheckpoint(playerid)
{
    if(IsPlayerInRangeOfPoint(playerid, 2.0, 2459.3779, -1689.3700, 13.5377) && g_pVehicleID[playerid] == INVALID_VEHICLE_ID && GetPlayerCurrentMission(playerid) == MISSION_HOME_INVASION)
    {
        ...
        if(hour > 3)
        {
            ShowPlayerSubtitle(playerid, "Misi ini hanya tersedia antara 00:00 dan 03:59.", 5000);
            return 1;
        }
        ...
        ShowPlayerSubtitle(playerid, "Pergi ke rumah Kolonel Fuhrberger.", 5000);
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    else
    {
        ...
        if(IsPlayerInRangeOfPoint(playerid, 5.0, x, y, z))
        {
            ...
            new ryderVoices[][][] =
            {
                {33665, 3000, "Apa yang saya katakan kalian semua? Kami Membunuh!"},
                {33666, 3000, "Kembali ke sana dan lepaskan tempat itu!"},
                {33667, 3000, "Ada banyak lagi di sana, Homie."},
                {33669, 3000, "Bawa dia untuk semua yang bisa Anda dapatkan!"},
                {33671, 3000, "Kami punya lebih banyak ruang di van LB!"},
                {33673, 3000, "Anda seorang pemecah rumah alami, homie!"}
            };
            ...
            {
                ShowPlayerSubtitle(playerid, "Bawa truk ke garasi.", 5000);
        ...
        else
        {
            ...
            gplTexts3D[playerid][i] = CreateDynamic3DTextLabel("Kotak\nTekan {1add69}Y", 0xFFFFFFFF, x, y, z-0.88, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid));
            RemovePlayerAttachedObject(playerid, gplAttachmentIndex[playerid]);
        }
    }
	return 1;
...
hook OnPlayerEnterRaceCPT(playerid)
{
    if(GetPlayerCurrentMission(playerid) == MISSION_HOME_INVASION)
    {
        if(!IsPlayerInVehicle(playerid, g_pVehicleID[playerid]))
        {
            ...
            ShowPlayerSubtitle(playerid, "Anda ~r~tidak~w~ di kendaraan misi.", 5000);
            return -2;
        }
        ...
        if(gplCurrentCP[playerid] == 0)
        {
            ...
            for(new i = 0; i < sizeof(gplObjects[]); i++)
            {
                ...
                gplTexts3D[playerid][i] = CreateDynamic3DTextLabel("Kotak\nTekan {1add69}Y", 0xFFFFFFFF, g_fObjectSpawns[i][0], g_fObjectSpawns[i][1], g_fObjectSpawns[i][2], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, playerid, 8);
            }
        ...
        else if(gplCurrentCP[playerid] == 1)
        {
            ...
            GameTextForPlayer(playerid, "misi berlalu!~n~~w~ hormat +", 9000, 0);

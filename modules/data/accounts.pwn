...
hook OnPlayerConnect(playerid)
{
    ...
    SendClientMessage(playerid, 0xA9C4E4FF, "Menghubungkan ke Database, Harap Tunggu...");
    return 1;
}
...
hook OnPlayerRequestSpawn(playerid)
{
    if(!IsPlayerLogged(playerid))
    {
        SendClientMessage(playerid, 0xb44819ff, "* Anda tidak sedang login.");
        return -1;
    }
    return 1;
}
...
public OnAccountCheck(playerid)
{
	...
	if(rows)
	{
        ...
        new info[104];
        format(info, sizeof(info), "Selamat datang kembali, %s!\n\nAkun Anda terdaftar.\nKetik kata sandi Anda untuk login.", playerName);
        ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login", info, "Masuk", "Keluar");
        PlaySelectSound(playerid);
	}
    else
    {
        new info[102];
        format(info, sizeof(info), "Selamat datang, %s!\n\nAkun Anda tidak terdaftar.\nKetikkan kata sandi untuk register.", playerName);
        ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_INPUT, "Register", info, "Masuk", "Keluar");
        PlaySelectSound(playerid);
    }
    return 1;
}
...
public OnAccountLoad(playerid)
{
	...
	if(rows)
	{
        ...
        cache_get_value_name_int(0, "lastlogin", gPlayerAccountData[playerid][e_player_lastlogin]);
